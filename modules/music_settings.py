...
class SkinSelector(disnake.ui.View):
    ...
    def rebuild_selects(self):
        ...
        global_mode = disnake.ui.Button(label=("Disable" if self.global_mode else "Enable") + " Global mode ", emoji="<a:tec_globe:1214597972285853706>")
        global_mode.callback = self.mode_callback
        self.add_item(global_mode)

        confirm_button = disnake.ui.Button(label="Save", emoji="<:tec_save:1216777209734369331>")
        confirm_button.callback = self.confirm_callback
        self.add_item(confirm_button)

        cancel_button = disnake.ui.Button(label="Cancel", emoji="<a:tec_cross:1213036478259535943>")
...
class PlayerSettings(disnake.ui.View):
    ...
    def load_buttons(self):
        ...
        check_other_bots_button = disnake.ui.Button(label="Do not connect with incompatible bots.",
                                                    emoji="<a:tec_tick:1213036220590854205>" if self.check_other_bots_in_vc else "<:tec_no:1215664168968134686>")
        ...
        restrict_mode_button = disnake.ui.Button(label="Restricted",
                                                    emoji="<a:tec_tick:1213036220590854205>" if self.enable_restrict_mode else "<:tec_no:1215664168968134686>")
        ...
        check_autoplay_button = disnake.ui.Button(label="Autoplay.",
                                                    emoji="<a:tec_tick:1213036220590854205>" if self.enable_autoplay else "<:tec_no:1215664168968134686>")
        check_autoplay_button.callback = self.autoplay_callback
        self.add_item(check_autoplay_button)

        close_button = disnake.ui.Button(label="Save/close", emoji="<:tec_save:1216777209734369331>")
...
class MusicSettings(commands.Cog):
    ...
    emoji = "<a:tec_setting:1212368345328783380>"
    name = "Settings"
    ...
    async def setup(
        ...
        if not target:
            ...
            buttons = [
                disnake.ui.Button(label="Create Text channel", custom_id=f"text_channel_{id_}", emoji="<:tec_speech:1215664691175497760>", disabled=not guild.me.guild_permissions.manage_channels),
                disnake.ui.Button(label="Create Voice Channel", custom_id=f"voice_channel_{id_}", emoji="<:tec_vol:1215664974622625902>", disabled=not guild.me.guild_permissions.manage_channels),
                disnake.ui.Button(label="Cancel", custom_id=f"voice_channel_cancel_{id_}", emoji="<a:tec_cross:1213036478259535943>")
            ]

            if "COMMUNITY" in guild.features:
                buttons.insert(2, disnake.ui.Button(label="Create Stage Channel", custom_id=f"stage_channel_{id_}",
                                  emoji="<:tec_stage:1222130324746797117>", disabled=not guild.me.guild_permissions.manage_channels))
            ...
            if delete_channel == "yes":
                ...
                await original_message.edit(
                    ...
                    embed=None, components=[
                        disnake.ui.Button(label="Reconfigure this channel", emoji="<a:tec_dot:1216394673896030289>",
    ...
    async def change_skin(self, interaction: disnake.AppCmdInter):
        ...
        skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>" if s.startswith("> custom_skin: ") else "<:tec_heart:1216398247535316993>", label=f"Normal mode: {s.replace('> custom_skin: ', '')}", value=s, **{"default": True, "description": "current skin"} if selected == s else {}) for s in skin_list + add_skin_prefix(global_data["custom_skins"])]
        static_skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>" if s.startswith("> custom_skin: ") else "<:tec_heart:1216398247535316993>", label=f"Song-Request: {s.replace('> custom_skin: ', '')}", value=s, **{"default": True, "description": "current skin"} if static_selected == s else {}) for s in static_skin_list + add_skin_prefix(global_data["custom_skins_static"])]

        global_skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>" if s.startswith("> custom_skin: ") else "<:tec_heart:1216398247535316993>", label=f"Normal mode: {s.replace('> custom_skin: ', '')}", value=s, **{"default": True, "description": "current skin"} if global_selected == s else {}) for s in skin_list + add_skin_prefix(global_data["custom_skins"])]
        global_static_skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>" if s.startswith("> custom_skin: ") else "<:tec_heart:1216398247535316993>", label=f"Song-Request: {s.replace('> custom_skin: ', '')}", value=s, **{"default": True, "description": "current skin"} if global_static_selected == s else {}) for s in static_skin_list + add_skin_prefix(global_data["custom_skins_static"])]
        ...
        ).set_image("https://media.discordapp.net/attachments/1162795987014787162/1203207132812546078/mjD6IaM.webp?ex=6610da76&is=65fe6576&hm=5a514fd0f39ac1c610c6bfbf7b98d37ed6bc233d89f21e933ce636df5abd2a9e&=&format=webp&width=1281&height=96")
        ...
        for b in self.bot.pool.bots:
            ...
            player.setup_hints()
            player.process_hint()
            player.set_command_log(text=f"{inter.author.mention} changed the Player skin.", emoji="<:tec_heart:1216398247535316993>")
    ...
    async def listenalong(self, ctx: CustomContext):
        ...
        await ctx.reply(
            ...
            ).set_image(url="https://media.discordapp.net/attachments/1162795987014787162/1216796040439795833/1.png?ex=6614259f&is=6601b09f&hm=941fbbd1368e8f7292e43ab7cea3baad3ccd4ae5fbf8274bc051a6e63dd4e38d&=&format=webp&quality=lossless&width=677&height=341").
    ...
    async def nodeinfo(self, interaction: disnake.AppCmdInter):
        ...
        for identifier, node in bot.music.nodes.items():
            ...
            if started:
                ...
                if players:
                    txt += f'<a:tec_play:1213064036606943254>`[{players}]`' + (" " if idle else "")
                if idle:
                    txt += f'<a:tec_afk:1213809464545910815>`[{idle}]`'
            ...
            status = "<:tec_star:1212069288203255948>" if current_player else "<a:tec_tick:1213036220590854205>"
        ...
        if failed_nodes:
            embeds.append(
                disnake.Embed(
                    title="**Servers that failed** `<a:tec_cross:1213036478259535943>`",
...
class RPCCog(commands.Cog):
    ...
    async def rich_presence(self, inter: disnake.AppCmdInter):
        ...
        if self.bot.config["ENABLE_RPC_AUTH"]:
            ...
            components.extend(
                [
                    disnake.ui.Button(label="Create/Reset token", custom_id=f"rpc_gen.{inter.author.id}", emoji="<:tec_key:1216391519758717030>",
                                      row=0),
                    ...
                    disnake.ui.Button(label="Remove token (Disable)", custom_id=f"rpc_remove.{inter.author.id}",
                                      emoji="<a:tec_recycle:1216392076833456203>", row=1),
                ]
            )
        ...
        if isinstance(inter, CustomContext):
            components.append(
                disnake.ui.Button(label="Close", custom_id=f"rpc_close.{inter.author.id}", emoji="<a:tec_cross:1213036478259535943>", row=1),
            )
