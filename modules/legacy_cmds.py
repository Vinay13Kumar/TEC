...
class Owner(commands.Cog):
    ...
    @commands.is_owner()
    @panel_command(aliases=["rd"], description="Reload modules.", emoji="<a:tec_reload:1214591681073254410>",
    ...
    async def update(self, ctx: Union[CustomContext, disnake.MessageInteraction], *,
        ...
        txt = f"`<a:tec_tick:1213036220590854205>` **[Update successful!]({self.bot.pool.remote_git_url}/commits/main)**"

        if git_log:
            txt += f"\n\n{self.format_log(git_log[:10])}"

        txt += f"\n\n`<:tec_page:1214593594078527499>` **Log:** ```py\n{out_git[:1000].split('Fast-forward')[-1]}```\n{text}"
    ...
    async def updatelog(self, ctx: Union[CustomContext, disnake.MessageInteraction], amount: int = 10):
        ...
        txt = f"<a:tec_misc:1212276940300623964> ** | [Recent Updates:]({self.bot.pool.remote_git_url}/commits/main)**\n\n" + self.format_log(
            git_log)
    ...
    @panel_command(aliases=["sync"], description="Manually sync slash commands.",
                emoji="<:tec_slash:1213019595494006824>",
    ...
    @panel_command(aliases=["expsource", "export", "exs"],
                description="Export my source code to a zip file.", emoji="<:tec_save:1216777209734369331>",
    ...
    async def close_shell_result(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.edit_message(
            content="```ini\n<a:tec_lock:1213021806697648128> - [Shell Closed!] - <a:tec_lock:1213021806697648128>```",
            attachments=None,
            view=None,
            embed=None
        )
    ...
    async def shell(self, ctx: CustomContext, *, command: str):
        ...
        try:
            ...
            try:
                ...
                await ctx.message.add_reaction("<a:tec_warn:1212266812998422528>")
            except disnake.Forbidden:
                traceback.print_exc()
                raise GenericError(
            ...
            await ctx.reply(
                components=[
                    disnake.ui.Button(label="Close Shell", custom_id="close_shell_result", emoji="<a:tec_recycle:1216392076833456203>")
                ],
    ...
    async def summon(self, ctx: CustomContext):
        ...
        if msg:
            ...
            self.bot.loop.create_task(ctx.message.add_reaction("<:tec_heart:1216398247535316993>"))
