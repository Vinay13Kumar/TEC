...
class Misc(commands.Cog):
    ...
    emoji = "<a:tec_misc:1212276940300623964>"
    name = "Miscellaneous"
    ...
    async def guild_add(self, guild: disnake.Guild):
        ...
        image = "https://media.discordapp.net/attachments/1162795987014787162/1203207132812546078/mjD6IaM.webp?ex=6610da76&is=65fe6576&hm=5a514fd0f39ac1c610c6bfbf7b98d37ed6bc233d89f21e933ce636df5abd2a9e&=&format=webp&width=1281&height=96"
    ...
    async def about(
        ...
        ram_msg = f"> <a:tec_desktop:1213035097838592020> **⠂RAM Usage (Python):** `{humanize.naturalsize(python_ram)}`\n"

        if lavalink_ram:
            ram_msg += f"> <a:tec_volcano:1213035926901624892> **⠂RAM Usage (Lavalink):** `{humanize.naturalsize(lavalink_ram)}`\n" \
                        f"> <a:tec_desktop:1213035097838592020> **⠂RAM Usage (Total):** `{humanize.naturalsize(python_ram + lavalink_ram)}`\n"
        ...
        for identifier, data in node_data.items():
            ...
            if data["available"] > 0:
                if data['website']:
                    nodes_available.add(
                        f"> [<a:tec_tick:1213036220590854205>`⠂{identifier}`]({data['website']}) `[{data['available']}/{data['total']}]`")
                else:
                    nodes_available.add(f"> <a:tec_tick:1213036220590854205>`⠂{identifier} [{data['available']}/{data['total']}]`")
            else:
                nodes_unavailable.add(f"> <a:tec_cross:1213036478259535943>`⠂{identifier}`")
        ...
        if len(bot.pool.bots) < 2:
            ...
            embed.description += "### Statistics (current bot):\n" \
                                 f"> <a:tec_server:1213063156671324170> **⠂Servers:** `{len(bot.guilds)}`\n" \
                                 f"> <a:tec_user:1213060775795625984> **⠂Users:** `{user_count:,}`\n"

            if bot_count:
                embed.description += f"> <:tec_bot:1213060278414221382> **⠂Bots:** `{bot_count:,}`\n"
            ...
            if public_bot_count:
                embed.description += f"> <:tec_bot:1213060278414221382> **⠂Public additional bot(s):** `{public_bot_count:,}`\n"

            if private_bot_count:
                embed.description += f"> <:tec_bot:1213060278414221382> **⠂Private additional bot(s):** `{private_bot_count:,}`\n"

            embed.description += f"> <a:tec_server:1213063156671324170> **⠂Servers:** `{guilds_size}`\n"

            if users_amount := len(users):
                embed.description += f"> <a:tec_user:1213060775795625984> **⠂Users:** `{users_amount:,}`\n"

            if bots_amount := len(bots):
                embed.description += f"> <:tec_bot:1213060278414221382> **⠂Bots:** `{bots_amount:,}`\n"

        embed.description += "### Other information:\n"

        if active_players_other_bots:
            embed.description += f"> <a:tec_play:1213064036606943254> **⠂Active players:** `{active_players_other_bots:,}`\n"

        if paused_players_other_bots:
            embed.description += f"> <:tec_pause:1213808106727800862> **⠂Players paused:** `{paused_players_other_bots:,}`\n"

        if inactive_players_other_bots:
            embed.description += f"> <a:tec_afk:1213809464545910815> **⠂Inactive players:** `{inactive_players_other_bots:,}`\n"

        if listeners:
            embed.description += f"> <a:tec_listener:1213811291744116796> **⠂Current listeners:** `{len(listeners):,}`\n"

        if bot.pool.commit:
            embed.description += f"> <:tec_ballot:1213852324234010734> **⠂Current commit:** [`{bot.pool.commit[:7]}`](https://discord.gg/jpvHPkfjaW)\n"

        embed.description += f"> <:tec_python:1213814966423916594> **⠂Python Version:** `{platform.python_version()}`\n" \
                            f"> <a:tec_disnake:1213852551577870407> **⠂Disnake Version:** `{disnake.__version__}`\n" \
                            f"> <a:tec_latency:1213813646472253481> **⠂Latency:** `{round(bot.latency * 1000)}ms`\n" \
                            f"{ram_msg}" \
                            f"> <a:tec_uptime:1213851690814668900> **⠂Uptime:** <t:{int(bot.uptime.timestamp())}:R>\n"
        ...
        if not bot.config["INTERACTION_COMMAND_ONLY"]:
            ...
            if guild_data["prefix"]:
                embed.description += f"> <a:tec_prefix:1213853442599030784> **⠂Server Prefix:** `{disnake.utils.escape_markdown(guild_data['prefix'], as_needed=True)}`\n"
            else:
                embed.description += f"> <a:tec_prefix:1213853442599030784> **⠂Default Prefix:** `{disnake.utils.escape_markdown(bot.default_prefix, as_needed=True)}`\n"
            ...
            if user_data["custom_prefix"]:
                embed.description += f"> <a:tec_prefix:1213853442599030784> **⠂Your User Prefix:** `{disnake.utils.escape_markdown(user_data['custom_prefix'], as_needed=True)}`\n"

        links = "[`[Join Server]`](https://discord.gg/jpvHPkfjaW)"

        if bot.config["SUPPORT_SERVER"]:
            links = f"[`[Support]`](https://discord.gg/jpvHPkfjaW)  **|** {links}"

        embed.description += f"> <a:tec_globe:1214597972285853706> **⠂**{links}\n"
