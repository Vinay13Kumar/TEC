...
class ServerManagerView(disnake.ui.View):
    ...
    def rebuild_components(self):
        ...
        if has_server_select and len(self.pages) > 1:
            ...
            back = disnake.ui.Button(label="Back", emoji="<:tec_left:1212066251967438929>")
            back.callback = self.previous_page
            self.add_item(back)

            next = disnake.ui.Button(label="Next", emoji="<:tec_right:1212066431978709082>")
            next.callback = self.next_page
            self.add_item(next)

        leave = disnake.ui.Button(label="Remove", emoji="<a:tec_recycle:1216392076833456203>", style=disnake.ButtonStyle.red)
        leave.callback = self.leave_guild
        self.add_item(leave)

        stop = disnake.ui.Button(label="Stop", emoji="<:tec_pause:1213808106727800862>", style=disnake.ButtonStyle.blurple)
