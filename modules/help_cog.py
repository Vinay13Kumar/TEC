    ...
    def process_buttons(self):
        ...
        if self.category:
            if len(self.cmd_lst[self.category]['cmds']) > 1:
                left_button = disnake.ui.Button(style=disnake.ButtonStyle.grey, emoji='<:tec_left:1212066251967438929>', custom_id="left_page")
                left_button.callback = self.callback_left
                self.add_item(left_button)

                right_button = disnake.ui.Button(style=disnake.ButtonStyle.grey, emoji='<:tec_right:1212066431978709082>', custom_id="right_page")
                right_button.callback = self.callback_right
                self.add_item(right_button)

            back_button = disnake.ui.Button(style=disnake.ButtonStyle.grey, emoji='<:tec_arrow_hook_left:1212066877820371025>', custom_id="back_page", label="Back")
...
class HelpCog(commands.Cog, name="Help"):
    ...
    async def get_cmd(self, ctx, cmds, index=0, category=None, emoji=None):
        ...
        txt = f"### <:tec_commands:1212067817558646805> ⠂Command: {ctx.prefix}{cmd}\n```\n{help_cmd}```\n"
        if cmd.aliases:
            aliases = " | ".join([f"{ctx.prefix}{ali}" for ali in cmd.aliases])
            txt += f"<:tec_add:1216398582609874996> **⠂Alternatives:** ```\n{aliases}```\n"
        if hasattr(cmd, 'commands'):
            subs = " | ".join([c.name for c in cmd.commands if (await check_perms(ctx, c))])
            txt += f"<:tec_sub_commands:1212069645868212254> **⠂Subcommands:** ```{subs}``` Use the command: `[ {ctx.prefix}help {cmd} subcommand ]` to see more details of the subcommand.\n\n"

        if usage_cmd:
            txt += f"<a:tec_book:1216413735330250864> **⠂How to Use:** ```\n{usage_cmd}```\n" \
                   f"<a:tec_warn:1212266812998422528> **⠂Notes on using arguments in the command:** ```\n" \
                   f"[] = Required | <> = Optional```\n"
        ...
        if flags and (actions := flags._actions):
            ...
            if t:
                txt += ("<a:tec_flag:1212269437760180234> **⠂Flags `(options to add at the end of the command)`:**```ini\n" + "\n\n".join(t) + "```")
    ...
    async def _help(self, ctx, *cmd_name):
        ...
        for cmd in sorted(ctx.bot.commands, key=lambda c: c.name):
            ...
            if category_icon:
                ...
                if not "<a:tec_misc:1212276940300623964>" in cmdlst:
                    cmdlst["<a:tec_misc:1212276940300623964>"] = ("Miscellaneous", [])
                cmdlst["<a:tec_misc:1212276940300623964>"][1].append(cmd)
    ...
    async def parse_direct(self, ctx: CustomContext, cmd_name: list):
        ...
        if any(c for c in cmd.cog.get_commands() if check_cmd(c)):
            ...
            emoji = cog.emoji if hasattr(cog, "emoji") else "<a:tec_misc:1212276940300623964>"
