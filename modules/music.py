...
class Music(commands.Cog):
    ...
    emoji = "<a:tec_play:1213064036606943254>"
    name = "Music"
    ...
    async def updatecache(self, ctx: CustomContext, *args):
        ...
        for url in list(self.bot.pool.playlist_cache):
            ...
            if not tracks:
                txt += f"[`<a:tec_cross:1213036478259535943> Failure`]({url})\n"
    ...
    async def stage_announce(
        ...
        if not template:
            ...
            player.set_command_log(text="activated automatic status", emoji="<a:tec_volu:1216405166459191428>")
    ...
    async def do_connect(
        ...
        if isinstance(ctx, disnake.AppCmdInter) and ctx.application_command.name == self.connect.name:
            ...
            await self.interaction_message(ctx, txt, emoji="<:tec_vol:1215664974622625902>", rpc_update=True)
            ...
            if not await check_deafen(me):
                await text_channel.send(
                    embed=disnake.Embed(
                        ...
                        url="https://media.discordapp.net/attachments/1162795987014787162/1216800161569968200/2.png?ex=66142975&is=6601b475&hm=4803573f3713cd98898f5c8c03ca62227fe6ca22bc7902c16d865eeecb81e1ef&=&format=webp&quality=lossless&width=425&height=542"
                    ), delete_after=20
                )
        ...
        if isinstance(channel, disnake.StageChannel):
            ...
            if stage_perms.mute_members:
                ...
                embed.set_footer(
                    text="<:tec_bulb:1216393991780831273> Tip: To allow me to speak on stage automatically, you will need to grant me the "
    ...
    async def play(
        ...
        if not query:
            ...
            opts = [
                disnake.SelectOption(label="Use favorite", value=">> [<:tec_star:1212069288203255948> Favorites <:tec_star:1212069288203255948>] <<", emoji="<:tec_star:1212069288203255948> "),
                disnake.SelectOption(label="Use integration", value=">> [<a:tec_dot:1216394673896030289> Integrations <a:tec_dot:1216394673896030289>] <<", emoji="<a:tec_dot:1216394673896030289>"),
            ]

            if os.path.isfile(f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl"):
                opts.append(disnake.SelectOption(label="Use saved queue", value=">> [<:tec_save:1216777209734369331> Saved Queue <:tec_save:1216777209734369331>] <<", emoji="<:tec_save:1216777209734369331>"))

            if user_data["last_tracks"]:
                opts.append(disnake.SelectOption(label="Add recent song", value=">> [<:tec_bookmark:1216397045741391974> Recent songs <:tec_bookmark:1216397045741391974>] <<", emoji="<:tec_bookmark:1216397045741391974>"))
            ...
            if guild_data["player_controller"]["fav_links"]:
                disnake.SelectOption(label="Use server favorite", value=">> [<:tec_pin:1216397286465343650> Server favorites <:tec_pin:1216397286465343650>] <<", emoji="<:tec_pin:1216397286465343650>"),
        ...
        if query.startswith(">> [<a:tec_dot:1216394673896030289> Integrations <a:tec_dot:1216394673896030289>] <<"):
            query = ""
            for k, v in user_data["integration_links"].items():
                fav_opts.append(disnake.SelectOption(label=k[5:], value=f"> itg: {k}", description="[<a:tec_dot:1216394673896030289> Integration <a:tec_dot:1216394673896030289>]", emoji=music_source_emoji_url(v)))

        elif query.startswith(">> [<:tec_star:1212069288203255948> Favorites <:tec_star:1212069288203255948>] <<"):
            query = ""
            for k, v in user_data["fav_links"].items():
                fav_opts.append(disnake.SelectOption(label=k, value=f"> fav: {k}", description="[<:tec_star:1212069288203255948> Favorite <:tec_star:1212069288203255948>]", emoji=music_source_emoji_url(v)))

        elif query.startswith(">> [<:tec_bookmark:1216397045741391974> Recent songs <:tec_bookmark:1216397045741391974>] <<"):
            ...
            for i, d in enumerate(user_data["last_tracks"]):
                fav_opts.append(disnake.SelectOption(label=d["name"], value=f"> lst: {i}", description="[<:tec_bookmark:1216397045741391974> Recent songs <:tec_bookmark:1216397045741391974>]",
                                                     emoji=music_source_emoji_url(d["url"])))

        elif query.startswith(">> [<:tec_pin:1216397286465343650> Server favorites <:tec_pin:1216397286465343650>] <<"):
            ...
            for name, v in guild_data["player_controller"]["fav_links"].items():
                fav_opts.append(disnake.SelectOption(label=name, value=f"> pin: {name}", description="[<:tec_pin:1216397286465343650> Server favorites <:tec_pin:1216397286465343650>]", emoji=music_source_emoji_url(v['url'])))
        ...
        elif query.startswith(">> [<:tec_save:1216777209734369331> Saved Queue <:tec_save:1216777209734369331>] <<"):
                ...
                if query.startswith(("https://youtu.be/", "https://www.youtube.com/")):
                    ...
                    if "&list=" in query and (link_re := YOUTUBE_VIDEO_REG.match(query)):
                        ...
                        view = SelectInteraction(
                            user=inter.author,
                            opts=[
                                disnake.SelectOption(label="Music", emoji="<a:tec_play:1213064036606943254>",
                                                     description="Load only the music from the link.", value="music"),
                                disnake.SelectOption(label="Playlist", emoji="<a:tec_play:1213064036606943254>",
        ...
        if isinstance(tracks, list):
            ...
            if not isinstance(tracks, list):
                ...
                duration = time_format(tracks.duration) if not tracks.is_stream else '<:tec_live:1216397982602231859> Livestream'
                ...
                embed.description = f"`{fix_characters(tracks.author, 15)}`**┃**`{time_format(tracks.duration) if not tracks.is_stream else '<:tec_live:1216397982602231859> Livestream'}`**┃**{inter.author.mention}"
                emoji = "<a:tec_music:1216415601778495558>"
                if reg_query is not None:
                    reg_query = {"name": tracks.title, "url": tracks.uri}
        ...
        if not is_pin:
            ...
            if loadtype == "track":
                components = [
                    disnake.ui.Button(emoji="<:tec_heart:1216398247535316993>", label="Favorite", custom_id=PlayerControls.embed_add_fav),
                    disnake.ui.Button(emoji="<:tec_play:1213064302647320626>", label="Play" + (" now" if (player.current and player.current.autoplay) else ""), custom_id=PlayerControls.embed_forceplay),
                    disnake.ui.Button(emoji="<:tec_add:1216398582609874996>", label="Add to queue",
    ...
    async def fav_add_autocomplete(self, inter: disnake.Interaction, query: str):
        ...
        favs = [">> [<:tec_star:1212069288203255948> Favorites <:tec_star:1212069288203255948>] <<", ">> [<a:tec_dot:1216394673896030289> Integrations <a:tec_dot:1216394673896030289>] <<", ">> [<:tec_pin:1216397286465343650> Server favorites <:tec_pin:1216397286465343650>] <<"]

        if os.path.isfile(f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl"):
            favs.append(">> [<:tec_save:1216777209734369331> Saved Queue <:tec_save:1216777209734369331>] <<")
    ...
    async def skip(
        ...
        if query:
            ...
            player.set_command_log(emoji="<:tec_skip:1216399066976489612>", text=f"{inter.author.mention} skipped to the current song.")

            embed = disnake.Embed(
                color=self.bot.get_color(guild.me),
                description= f"<:tec_skip:1216399066976489612> **⠂{inter.author.mention} skipped to the song:**\n"
            ...
            if isinstance(inter, disnake.MessageInteraction):
                player.set_command_log(text=f"{inter.author.mention} skipped the song.", emoji="<:tec_skip:1216399066976489612>")
                if not inter.response.is_done():
                    try:
                        await inter.response.defer()
                    except:
                ...
                player.set_command_log(emoji="<:tec_skip:1216399066976489612>", text=f"{inter.author.mention} skipped the song.")

                embed = disnake.Embed(
                    color=self.bot.get_color(guild.me),
                    description=f"<:tec_skip:1216399066976489612> **⠂{inter.author.mention} skipped the song:\n"
    ...
    async def back(self, inter: disnake.AppCmdInter):
        ...
        if not len(player.queue) and (player.keep_connected or not len(player.played)):
            await player.seek(0)
            await self.interaction_message(inter, "Went back to the beginning of the song.", emoji="<:tec_rewind:1216401126690132190>")
            return
        ...
        if isinstance(inter, disnake.MessageInteraction):
            interaction = inter
            player.set_command_log(text=f"{inter.author.mention} returned to the current song.", emoji="<:tec_prev:1216400464556462221>")
            await inter.response.defer()
        else:
            ...
            txt = [
                ...
                f"<:tec_prev:1216400464556462221> **⠂{inter.author.mention} returned to the song:\n╰[`{fix_characters(t.title, 43)}`]({t.uri or t.search_uri})**"
            ]

            await self.interaction_message(inter, txt, emoji="<:tec_prev:1216400464556462221>", store_embed=True)
    ...
    async def voteskip(self, inter: disnake.AppCmdInter):
        ...
        if len(player.votes) < self.bot.config.get('VOTE_SKIP_AMOUNT', 3):
            ...
            await self.interaction_message(inter, txt, emoji="<:tec_hand:1216401473194295297>")
            return
    ...
    async def volume(
        ...
        txt = [f"adjusted the volume to **{value}%**", f"<:tec_vol:1215664974622625902> **⠂{inter.author.mention} adjusted the volume to {value}%**"]
        await self.interaction_message(inter, txt, emoji="<:tec_vol:1215664974622625902>")
    ...
    async def pause(self, inter: disnake.AppCmdInter):
        ...
        txt = ["paused the music.", f"<:tec_pause:1213808106727800862> **⠂{inter.author.mention} paused the music.**"]

        await self.interaction_message(inter, txt, rpc_update=True, emoji="<:tec_pause:1213808106727800862>")
        await player.update_stage_topic()
    ...
    async def resume(self, inter: disnake.AppCmdInter):
        ...
        txt = ["resumed the music.", f"<:tec_PlayPause:1216414413922504794> **⠂{inter.author.mention} unpaused the music.**"]
        await self.interaction_message(inter, txt, rpc_update=True, emoji="<:tec_PlayPause:1216414413922504794>")
        await player.update_stage_topic()
    ...
    async def seek(
        ...
        if milliseconds > player.position:
            ...
            emoji = "<:tec_for:1216405564150517780>"
            ...
            emoji = "<:tec_rewind:1216401126690132190>"
    ...
    async def loop_amount(
        ...
        txt = [
            ...
            f"<a:tec_loop1:1216407116584325241> **⠂{inter.author.mention} set the number of loops for the song to [{value}]:**\n"
            f"╰[`{player.current.title}`]({player.current.uri or player.current.search_uri})"
        ]

        await self.interaction_message(inter, txt, rpc_update=True, emoji="<a:tec_loop1:1216407116584325241>")
    ...
    async def remove(
        ...
        txt = [
            ...
            f"<a:tec_recycle:1216392076833456203> **⠂{inter.author.mention} removed the song from the queue:**\n╰[`{track.title}`]({track.uri or track.search_uri})"
        ]

        await self.interaction_message(inter, txt, emoji="<a:tec_recycle:1216392076833456203>")
    ...
    async def readd_songs(self, inter: disnake.AppCmdInter):
        ...
        txt = [
            ...
            f"<a:tec_play:1213064036606943254> **⠂{inter.author.mention} re-added {qsize} song{s} to the queue.**"
        ]

        await self.interaction_message(inter, txt, emoji="<a:tec_play:1213064036606943254>")
    ...
    async def rotate(
        ...
        txt = [
            ...
            f"<a:tec_reload:1214591681073254410> **⠂{inter.author.mention} rotated the queue to the song:**\n╰[`{track.title}`]({track.uri or track.search_uri})."
        ]

        if isinstance(inter, disnake.MessageInteraction):
            player.set_command_log(text=f"{inter.author.mention} " + txt[0], emoji="<a:tec_reload:1214591681073254410>")
        else:
            await self.interaction_message(inter, txt, emoji="<a:tec_reload:1214591681073254410>")
    ...
    async def song_request_thread(self, inter: disnake.AppCmdInter):
        ...
        txt = [
            ...
            f"<:tec_speech:1215664691175497760> **⠂{inter.author.mention} created a [thread/conversation]({thread.jump_url}) temporary for song requests.**"
        ]

        await self.interaction_message(inter, txt, emoji="<:tec_speech:1215664691175497760>", defered=True, force=True)
    ...
    async def nightcore(self, inter: disnake.AppCmdInter):
        ...
        txt = [f"{txt} the nightcore effect.", f"<a:tec_moon:1220632480340508735> **⠂{inter.author.mention} {txt} the nightcore effect.**"]

        await self.interaction_message(inter, txt, emoji="<a:tec_moon:1220632480340508735>")
    ...
    async def now_playing(self, inter: disnake.AppCmdInter):
        ...
        if player.current.is_stream:
            txt += "> <:tec_live:1216397982602231859> **⠂Live broadcast**\n"
        else:
        ...
        txt += f"> <a:tec_user:1213060775795625984> **⠂Uploader/Artist(s):** `{player.current.authors_md}`\n"

        if player.current.album_name:
            txt += f"> <a:tec_disc:1213021509564764170> **⠂Álbum:** [`{fix_characters(player.current.album_name, limit=20)}`]({player.current.album_url})\n"

        if not player.current.autoplay:
            txt += f"> <:tec_hand:1216401473194295297> **⠂Requested by:** <@{player.current.requester}>\n"
        else:
            try:
        ...
        try:
            txt += f"> <a:tec_play:1213064036606943254> **⠂Voice channel:** {player.guild.me.voice.channel.jump_url}\n"
        except AttributeError:
            pass

        txt += f"> <:tec_vol:1215664974622625902> **⠂Volume:** `{player.volume}%`\n"

        components = [disnake.ui.Button(custom_id=f"np_{inter.author.id}", label="Update", emoji="<a:tec_loop1:1216407116584325241>")]
        ...
        if player.guild_id != inter.guild_id:
            ...
            if (listeners:=len([m for m in player.last_channel.members if not m.bot and (not m.voice.self_deaf or not m.voice.deaf)])) > 1:
                txt += f"> <a:tec_listener:1213811291744116796> **⠂Current listeners:** `{listeners}`\n"

            txt += f"> <a:tec_uptime:1213851690814668900> **⠂Active player:** <t:{player.uptime}:R>\n"
        ...
        if player.keep_connected:
            txt += "> <a:tec_loop1:1216407116584325241> **⠂24/7 mode:** `Activated`\n"

        if player.queue or player.queue_autoplay:

            if player.guild_id == inter.guild_id:

                txt += f"### <a:tec_play:1213064036606943254> ⠂Next songs ({(qsize := len(player.queue + player.queue_autoplay))}):\n" + (
                            "\n> `" + ("-" * 38) + "`\n").join(
                    f"> `{n + 1})` [`{fix_characters(t.title, limit=38)}`]({t.uri})\n" \
                    f"> `<a:tec_uptime:1213851690814668900> {time_format(t.duration) if not t.is_stream else '<:tec_live:1216397982602231859> Live'}`" + (
                        f" - `Repetitions: {t.track_loops}`" if t.track_loops else "") + \
                    f" **|** " + (f"`<:tec_hand:1216401473194295297>` <@{t.requester}>" if not t.autoplay else f"`<:tec_heart:1216398247535316993>⠂Recommended`") for n, t in
                ...
                if qsize > 3:
                    components.append(
                        disnake.ui.Button(custom_id=PlayerControls.queue, label="See full list",
                                          emoji="<:tec_queue:1217105407038722108>"))

            elif player.queue:
                txt += f"> <a:tec_play:1213064036606943254> **⠂Songs in the queue:** `{len(player.queue)}`\n"
        ...
        if channel != player.text_channel:
            ...
            try:
                ...
                player.set_command_log(
                    ...
                    emoji="<a:tec_dot:1216394673896030289>"
                )

                embed = disnake.Embed(
                    description=f"<a:tec_dot:1216394673896030289> **⠂{inter.author.mention} moved the player-controller to the channel:** {channel.mention}",
                    color=self.bot.get_color(guild.me)
                )
    ...
    async def add_dj(
        ...
        text = [f"added {user.mention} to the DJ list.",
                f"<a:tec_listener:1213811291744116796> **⠂{inter.author.mention} added {user.mention} to the DJ list.**"]
        ...
        await self.interaction_message(inter, txt=text, emoji="<a:tec_listener:1213811291744116796>")
    ...
    async def remove_dj(
        ...
        text = [f"removed {user.mention} from the DJ list.",
                f"<a:tec_listener:1213811291744116796> **⠂{inter.author.mention} removed {user.mention} from the DJ list.**"]
    ...
    async def stop(self, inter: disnake.AppCmdInter):
        ...
        if isinstance(inter, disnake.MessageInteraction):
            ...
            embed = disnake.Embed(
                color=self.bot.get_color(guild.me),
                description=f"<:tec_stop:1216409401217515550> **⠂{inter.author.mention} stopped the player.**"
            )
    ...
    async def shuffle_(self, inter: disnake.AppCmdInter):
        ...
        await self.interaction_message(
            inter,
            ["shuffled the queue.",
             f"<:tec_shuffle:1216409744978350080> **⠂{inter.author.mention} shuffled the queue.**"],
            emoji="<:tec_shuffle:1216409744978350080>"
        )
    ...
    async def reverse(self, inter: disnake.AppCmdInter):
        ...
        await self.interaction_message(
            ...
            txt=["reversed the order of songs in the queue.",
                 f"<a:tec_loop1:1216407116584325241> **⠂{inter.author.mention} reversed the order of songs in the queue.**"],
            emoji="<a:tec_loop1:1216407116584325241>"
        )
    ...
    async def clear(
        ...
        if not filters and not range_start and not range_end:
            player.queue.clear()
            txt = ['cleared the music queue.', f'<a:tec_recycle:1216392076833456203> **⠂{inter.author.mention} cleared the music queue.**']
            ...
            msg_txt = f"### <a:tec_recycle:1216392076833456203> ⠂{inter.author.mention} removed {deleted_tracks} song{'s'[:deleted_tracks^1]} from the queue:\n" + "\n".join(f"[`{fix_characters(t.title, 45)}`]({t.uri})" for t in tracklist[:7])

            if (trackcount:=(len(tracklist) - 7)) > 0:
                msg_txt += f"\n`and {trackcount} more song{'s'[:trackcount^1]}.`"

            msg_txt += f"\n### <a:tec_tick:1213036220590854205> ⠂Filter{(t:='s'[:len(txt)^1])} used{t}:\n" + '\n'.join(txt)
        ...
        await self.interaction_message(inter, txt, emoji="<a:tec_recycle:1216392076833456203>", **kwargs)
    ...
    async def do_move(
        ...
        if indexes:
            ...
            txt = [
                ...
                f"<:tec_arrow_hook_left:1212066877820371025> **⠂{inter.author.mention} moved a song to position [{position}]:**\n"
                f"╰[`{fix_characters(track.title, limit=43)}`]({track.uri or track.search_uri})"
            ]

            await self.interaction_message(inter, txt, emoji="<:tec_arrow_hook_left:1212066877820371025>")
            ...
            msg_txt = f"### <:tec_arrow_hook_left:1212066877820371025> ⠂{inter.author.mention} moved {moved_tracks} song{'s'[:moved_tracks^1]} to position {moved_tracks_txt} in the queue:\n" + "\n".join(f"`{position+n}.` [`{fix_characters(t.title, 45)}`]({t.uri})" for n, t in enumerate(tracklist[:7]))
            ...
            await self.interaction_message(inter, txt, emoji="<:tec_arrow_hook_left:1212066877820371025>", force=True, thumb=tracklist[0].thumb)
    ...
    async def restrict_mode(self, inter: disnake.AppCmdInter):
        ...
        msg = ["activated", "<a:tec_tick:1213036220590854205>"] if player.restrict_mode else ["disabled", "<a:tec_cross:1213036478259535943>"]
    ...
    async def nonstop(self, inter: disnake.AppCmdInter):
        ...
        msg = ["activated", "<a:tec_tick:1213036220590854205>"] if player.keep_connected else ["deactivated", "<a:tec_cross:1213036478259535943>"]
    ...
    async def autoplay(self, inter: disnake.AppCmdInter):
        ...
        msg = ["enabled", "<a:tec_loop1:1216407116584325241>"] if player.autoplay else ["disabled", "<a:tec_cross:1213036478259535943>"]
    ...
    async def change_node(
        ...
        player.set_command_log(
            ...
            emoji="<a:tec_globe:1214597972285853706>"
        )
        ...
        if control in (
            ...
            try:
                ...
                if control == PlayerControls.embed_enqueue_playlist:
                    ...
                    track: Optional[Union[LavalinkTrack, PartialTrack]] = None
                    seek_status = False
                    ...
                    if player:
                        ...
                        if control == PlayerControls.embed_forceplay and player.current and (player.current.uri.startswith(url) or url.startswith(player.current.uri)):
                            ...
                            await player.seek(0)
                            player.set_command_log("Returned to the beginning of the song.", emoji="<:tec_rewind:1216401126690132190>")
        ...
        try:
            ...
            if control == PlayerControls.enqueue_fav:
                ...
                if control == PlayerControls.help_button:
                    embed = disnake.Embed(
                        description="<a:tec_book:1216413735330250864> **BUTTON INFORMATION** <a:tec_book:1216413735330250864>\n\n"
                                    "<:tec_PlayPause:1216414413922504794> `= Pause/Resume the music.`\n"
                                    "<:tec_prev:1216400464556462221> `= Go back to the previously played music.`\n"
                                    "<:tec_skip:1216399066976489612> `= Skip to the next music.`\n"
                                    "<:tec_shuffle:1216409744978350080> `= Shuffle the queue.`\n"
                                    "<a:tec_play:1213064036606943254> `= Add music/playlist/favorite.`\n"
                                    "<:tec_stop:1216409401217515550> `= Stop the player and disconnect me from the channel.`\n"
                                    "<:tec_bookmark:1216397045741391974> `= Display the music queue.`\n"
                                    "<a:tec_setting:1212368345328783380> `= Change some player settings:`\n"
                ...
                if control == PlayerControls.miniqueue:
                    ...
                    player.set_command_log(
                        emoji="<:tec_bookmark:1216397045741391974>",
                ...
                if control == PlayerControls.add_favorite:
                    ...
                    if player.current.album_url:
                        choices["Album"] = {
                            ...
                            "emoji": "<a:tec_disc:1213021509564764170>"
                        }
    ...
    async def process_music(
        ...
        if not player.current:
            if warn_message:
                player.set_command_log(emoji="<a:tec_warn:1212266812998422528>", text=warn_message)
            await player.process_next()
        elif force_play == "yes":
            player.set_command_log(
                emoji="<:tec_add:1216398582609874996>",
    ...
    async def parse_song_request(self, message: disnake.Message, text_channel, data, *, response=None, attachment: disnake.Attachment=None, source=None):
        ...
        try:
            components = [disnake.ui.Button(emoji="<a:tec_music:1216415601778495558>", label="Player-controller", url=player.message.jump_url)]
        except AttributeError:
            components = []
        ...
        if not isinstance(tracks, list):
            ...
            if (isinstance(message.channel, disnake.Thread) and
                ...
                embed.description = f"<:tec_hand:1216401473194295297> **⠂ Requested by:** {message.author.mention}\n" \
                                    f"<:tec_music:1217100697984696340> **⠂ Track{'s'[:tcount^1]}:** `[{tcount}]`"
                embed.set_thumbnail(url=tracks.tracks[0].thumb)
                embed.set_author(name="⠂" + fix_characters(tracks.tracks[0].playlist_name, 35), url=message.content,
                                 icon_url=music_source_image(tracks.tracks[0].info["sourceName"]))

                try:
                    embed.description += f"\n<:tec_vol:1215664974622625902> **⠂ Voice channel:** {message.author.voice.channel.mention}"
                except AttributeError:
                    pass
                ...
                components.extend(
                    [
                        disnake.ui.Button(emoji="<:tec_heart:1216398247535316993>", label="Favorite", custom_id=PlayerControls.embed_add_fav),
                        disnake.ui.Button(emoji="<:tec_add:1216398582609874996>", label="Add to queue",custom_id=PlayerControls.embed_enqueue_playlist)
                    ]
                )
                ...
                txt = f"> <:tec_music:1217100697984696340> **⠂** [`{fix_characters(tracks.tracks[0].playlist_name, 35)}`](<{message.content}>) `[{(tcount:=len(tracks.tracks))} track{'s'[:tcount^1]}]` {message.author.mention}"
            ...
            if (isinstance(message.channel, disnake.Thread) and
                    (not isinstance(message.channel.parent, disnake.ForumChannel) or
                     data['player_controller']['purge_mode'] != SongRequestPurgeMode.on_message)):
                embed.description = f"<a:tec_dot:1216394673896030289> **⠂ Uploader:** `{track.author}`\n" \
                                    f"<:tec_hand:1216401473194295297> **⠂ Requested by:** {message.author.mention}\n" \
                                    f"<a:tec_uptime:1213851690814668900> **⠂ Duration:** `{time_format(track.duration) if not track.is_stream else '<:tec_live:1216397982602231859> Livestream'}`"
                ...
                txt = f"> <a:tec_play:1213064036606943254> **⠂** [`{fix_characters(track.title, 35)}`](<{track.uri}>) `[{time_format(track.duration) if not track.is_stream else '<:tec_live:1216397982602231859> Livestream'}]` {message.author.mention}"
                ...
                duration = time_format(tracks[0].duration) if not tracks[0].is_stream else '<:tec_live:1216397982602231859> Livestream'
                player.set_command_log(
    ...
    async def interaction_message(self, inter: Union[disnake.Interaction, CustomContext], txt, emoji: str = "<a:tec_tick:1213036220590854205>",
        ...
        if max_retries:
            ...
            while not self.bot.is_closed():
                if retries >= max_retries:
                    print(
                        f"<a:tec_cross:1213036478259535943> - {self.bot.user} - All attempts to connect to the server [{data['identifier']}] failed.\n"
                    ...
                    try:
                        ...
                        if data["identifier"] != "LOCAL":
                            print(f'<a:tec_warn:1212266812998422528> - {self.bot.user} - Failed to connect to the server [{data["identifier"]}], '
            ...
            try:
                ...
                print(f"<a:tec_cross:1213036478259535943> - {self.bot.user} - Failed to connect to the server {data['identifier']}: {repr(e)}"[:300])
                return
                ...
                try:
                    ...
                    if before.channel.permissions_for(member.guild.me).connect:
                        ...
                        player.set_command_log(text="The player was terminated because I was moved to the channel "
                                                    ...
                                                    "my multi-voice system.", emoji="<a:tec_warn:1212266812998422528>")
                        await player.destroy()
                    return
                except AttributeError:
                    pass
