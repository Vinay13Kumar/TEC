...
class QueueInteraction(disnake.ui.View):
    ...
    async def on_timeout(self) -> None:
        ...
        self.embed.set_footer(text="Time to interact is over!")
    ...
    def update_components(self):
        ...
        first = disnake.ui.Button(emoji='<:tec_prev:1216400464556462221>', style=disnake.ButtonStyle.grey)
        first.callback = self.first
        self.add_item(first)

        back = disnake.ui.Button(emoji='<:tec_left:1212066251967438929>', style=disnake.ButtonStyle.grey)
        back.callback = self.back
        self.add_item(back)

        next = disnake.ui.Button(emoji='<:tec_right:1220240027959955530>', style=disnake.ButtonStyle.grey)
        next.callback = self.next
        self.add_item(next)

        last = disnake.ui.Button(emoji='<:tec_next:1216400005972365345>', style=disnake.ButtonStyle.grey)
        last.callback = self.last
        self.add_item(last)

        stop_interaction = disnake.ui.Button(emoji='<:tec_stop:1216409401217515550>', style=disnake.ButtonStyle.grey)
        stop_interaction.callback = self.stop_interaction
        self.add_item(stop_interaction)

        play = disnake.ui.Button(emoji='<:tec_play:1213064302647320626>', label="Play", style=disnake.ButtonStyle.grey, custom_id="queue_skip")
        play.callback = self.invoke_command
        self.add_item(play)

        move = disnake.ui.Button(emoji="<:tec_arrow_hook_left:1212066877820371025>", label="Move", style=disnake.ButtonStyle.grey, custom_id="queue_move")
        move.callback = self.move_callback
        self.add_item(move)

        rotate_q = disnake.ui.Button(emoji='<a:tec_reload:1214591681073254410>', label="Rotate Queue", style=disnake.ButtonStyle.grey, custom_id="queue_rotate")
        rotate_q.callback = self.invoke_command
        self.add_item(rotate_q)

        update_q = disnake.ui.Button(emoji='<:tec_Loop:1216406941191114793>', label="Reload", style=disnake.ButtonStyle.grey)
    ...
    def update_embed(self):
        ...
        for n, t in enumerate(self.track_pages[self.current_page]):
            ...
            duration = time_format(t.duration) if not t.is_stream else '<:tec_live:1216397982602231859> Livestream'
            ...
            if self.current_track == t:
                txt += f"`╔{'='*50}`\n`║` **{index}º) [{fix_characters(t.title, limit=37)}]({t.uri})**\n" \
                       ...
                       " **|** " + (f"`<:tec_hand:1216401473194295297>` <@{t.requester}>" if not t.autoplay else f"`<:tec_heart:1216398247535316993>⠂Recommended`") + f"\n`╚{'='*50}`\n"
            else:
                txt += f"`┌ {index})` [`{fix_characters(t.title, limit=45)}`]({t.uri})\n" \
                       f"`└ <a:tec_uptime:1213851690814668900> {duration}`" + (f" - `Repetitions: {t.track_loops}`" if t.track_loops else "") + \
                       f" **|** " + (f"`<:tec_hand:1216401473194295297>` <@{t.requester}>" if not t.autoplay else f"`<:tec_heart:1216398247535316993>⠂Recommended`") + "\n"
        ...
        try:
            ...
            if interaction.data.custom_id == "queue_skip":
                if player.current and player.current.unique_id == self.current_track.unique_id:
                    ...
                    await player.seek(0)
                    player.set_command_log(emoji="<:tec_rewind:1216401126690132190>", text=f"{interaction.author.mention} rewinded music to: `0:00`")
                    player.update = True
...
class SelectInteraction(disnake.ui.View):
    ...
    def load_components(self):
        ...
        if len(self.item_pages) > 1:
            ...
            back_button = disnake.ui.Button(emoji="<:tec_left:1212066251967438929>")
            back_button.callback = self.back_callback
            self.add_item(back_button)

            next_button = disnake.ui.Button(emoji="<:tec_right:1220240027959955530>")
            next_button.callback = self.next_callback
            self.add_item(next_button)

        button = disnake.ui.Button(label="Cancel", emoji="<a:tec_cross:1213036478259535943>")
...
class AskView(disnake.ui.View):
    ...
    @disnake.ui.button(label="Yes", emoji="<a:tec_tick:1213036220590854205>")
    async def allow(self, button, interaction: disnake.MessageInteraction):
        self.selected = True
        self.interaction_resp = interaction
        self.stop()

    @disnake.ui.button(label="No", emoji="<a:tec_cross:1213036478259535943>")
...
class FavMenuView(disnake.ui.View):
    ...
    def update_components(self):
        ...
        mode_select = disnake.ui.Select(
            options=[
                disnake.SelectOption(label="Favorites Manager", value=f"fav_view_mode_{ViewMode.fav_manager}", emoji="<:tec_star:1212069288203255948>",
        ...
        if self.bot.config["USE_YTDL"] or self.bot.spotify:
            mode_select.append_option(
                disnake.SelectOption(label="Integrations Manager", value=f"fav_view_mode_{ViewMode.integrations_manager}", emoji="<a:tec_dot:1216394673896030289>",
        ...
        if self.ctx.author.guild_permissions.manage_guild:
            mode_select.options.insert(1, disnake.SelectOption(label="Server Playlists Manager",
                                                               value=f"fav_view_mode_{ViewMode.guild_fav_manager}", emoji="<:tec_pin:1216397286465343650>",
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            remove_button = disnake.ui.Button(label="Remove", emoji="<a:tec_recycle:1216392076833456203>", disabled=not self.data["fav_links"])
            remove_button.callback = self.remove_callback
            self.add_item(remove_button)

            clear_button = disnake.ui.Button(label="Clean favorites", emoji="<:tec_bin:1221374851491823687>", disabled=not self.data["fav_links"])
            clear_button.callback = self.clear_callback
            self.add_item(clear_button)

            export_button = disnake.ui.Button(label="Export", emoji="<:tec_ballot:1213852324234010734>", disabled=not self.data["fav_links"])
            ...
            remove_button = disnake.ui.Button(label="Remove", emoji="<a:tec_recycle:1216392076833456203>", disabled=not self.guild_data["player_controller"]["fav_links"])
            remove_button.callback = self.remove_callback
            self.add_item(remove_button)

            clear_button = disnake.ui.Button(label="Clean favorites", emoji="<:tec_bin:1221374851491823687>", disabled=not self.guild_data["player_controller"]["fav_links"])
            clear_button.callback = self.clear_callback
            self.add_item(clear_button)

            export_button = disnake.ui.Button(label="Export", emoji="<:tec_ballot:1213852324234010734>", disabled=not self.guild_data["player_controller"]["fav_links"])
            export_button.callback = self.export_callback
            self.add_item(export_button)

        elif self.mode == ViewMode.integrations_manager:
            remove_button = disnake.ui.Button(label="Remove", emoji="<a:tec_recycle:1216392076833456203>", disabled=not self.data["integration_links"])
            remove_button.callback = self.remove_callback
            self.add_item(remove_button)

            clear_button = disnake.ui.Button(label="Clean Integrations", emoji="<:tec_bin:1221374851491823687>", disabled=not self.data["integration_links"])
            clear_button.callback = self.clear_callback
            self.add_item(clear_button)

            export_button = disnake.ui.Button(label="Export", emoji="<:tec_ballot:1213852324234010734>", disabled=not self.data["integration_links"])
            export_button.callback = self.export_callback
            self.add_item(export_button)

        import_button = disnake.ui.Button(label="Import", emoji="<:tec_ballot:1213852324234010734>")
        import_button.callback = self.import_callback
        self.add_item(import_button)

        if self.mode == ViewMode.fav_manager:
            play_button = disnake.ui.Button(label="Play selected favorite", emoji="<:tec_play:1213064302647320626>", custom_id="favmanager_play_button")
            play_button.callback = self.play_callback
            self.add_item(play_button)

        elif self.mode == ViewMode.integrations_manager:
            if self.data["integration_links"]:
                play_button = disnake.ui.Button(label="Play a playlist from selected integration", emoji="<:tec_play:1213064302647320626>", custom_id="favmanager_play_button")
                play_button.callback = self.play_callback
                self.add_item(play_button)

        cancel_button = disnake.ui.Button(label="Close", emoji="<a:tec_cross:1213036478259535943>")
...
class SkinSettingsButton(disnake.ui.View):
    ...
    def update_components(self):
        ...
        if self.mode == "custom_skins":
            controller_btn = disnake.ui.Button(emoji="<a:tec_dot:1216394673896030289>",
        ...
        save_btn = disnake.ui.Button(label="Save", emoji="<:tec_save:1216777209734369331>")
...
class SetStageTitle(disnake.ui.View):
    ...
    @disnake.ui.button(emoji='<:tec_vol:1215664974622625902>', style=disnake.ButtonStyle.grey, label="Enable/Disable status")
    ...
    @disnake.ui.button(emoji='<:tec_save:1216777209734369331>', style=disnake.ButtonStyle.grey, label="Enable/Disable status (permanent)")
        ...
        if inter.data.custom_id == "status_voice_channel_perm":
            ...
            for b in self.bot.pool.bots:
                ...
                p.set_command_log(
                    ...
                    emoji="<:tec_speaker:1221118288889778349>",
                )
                p.update = True
...
class SkinEditorMenu(disnake.ui.View):
    ...
    def update_components(self):
        ...
        if self.mode == "select":
            ...
            skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>", label=f"Normal Mode: {s.replace('> cs: ', '', 1)}", value=s) for s in add_skin_prefix(self.global_data["custom_skins"])]
            add_skin_prefix = (lambda d: [f"> css: {i}" for i in d.keys()])
            static_skins_opts = [disnake.SelectOption(emoji="<a:tec_dot:1216394673896030289>", label=f"Song-Request: {s.replace('> css: ', '', 1)}", value=s) for s in add_skin_prefix(self.global_data["custom_skins_static"])]
            ...
            if embeds:=self.message_data.get("embeds"):
                ...
                if len(fields) < 25:
                    add_field_btn = disnake.ui.Button(label="Add Field", emoji="<:tec_bookmark:1216397045741391974>")
                    add_field_btn.callback = self.add_field
                    self.add_item(add_field_btn)

                if fields:
                    edit_field_btn = disnake.ui.Button(label="Edit Field", emoji="<:tec_bookmark:1216397045741391974>")
                    edit_field_btn.callback = self.edit_embed_field_button
                    self.add_item(edit_field_btn)

                    delete_field_btn = disnake.ui.Button(label="Remove Field", emoji="<:tec_bookmark:1216397045741391974>")
                    delete_field_btn.callback = self.delete_embed_field_button
                    self.add_item(delete_field_btn)

                edit_embed_btn = disnake.ui.Button(label="Edit Embed", emoji="<:tec_page:1214593594078527499>")
                edit_embed_btn.callback = self.edit_embed_button
                self.add_item(edit_embed_btn)

                remove_embed_btn = disnake.ui.Button(label="Remove Embed", emoji="<:tec_page:1214593594078527499>")
                remove_embed_btn.callback = self.remove_embed
                self.add_item(remove_embed_btn)

                set_author_footer_btn = disnake.ui.Button(label="Embed Author + Footer", emoji="<a:tec_user:1213060775795625984>")
                set_author_footer_btn.callback = self.set_author_footer
                self.add_item(set_author_footer_btn)

            edit_content_btn = disnake.ui.Button(label=("Add" if not self.message_data.get("content") else "Edit") + " Message", emoji="<:tec_speech:1215664691175497760>")
            edit_content_btn.callback = self.edit_content
            self.add_item(edit_content_btn)

            add_embed_btn = disnake.ui.Button(label="Add Embed", disabled=len(embeds)>=8, emoji="<:tec_page:1214593594078527499>")
            add_embed_btn.callback = self.add_embed
            self.add_item(add_embed_btn)

            setup_queue_btn = disnake.ui.Button(label="Setup Queue Placeholder", emoji="<:tec_queue:1217105407038722108>")
            ...
            export_btn = disnake.ui.Button(label="Export Skin", emoji="<:tec_ballot:1213852324234010734>", disabled=save_disabled)
            export_btn.callback = self.export
            self.add_item(export_btn)

            import_btn = disnake.ui.Button(label="Import Skin", emoji="<:tec_ballot:1213852324234010734>")
            import_btn.callback = self.import_
            self.add_item(import_btn)

            if self.skin_selected:
                delete_skin_btn = disnake.ui.Button(label="Delete Skin", emoji="<:tec_bin:1221374851491823687>")
                delete_skin_btn.callback = self.delete_skin
                self.add_item(delete_skin_btn)

            back_btn = disnake.ui.Button(label="Go back to previous menu", emoji="<:tec_left:1212066251967438929>")
            back_btn.callback = self.back
            self.add_item(back_btn)

            self.add_item(disnake.ui.Button(label="List of placeholders", emoji="<a:tec_ques:1212267390910468127>", custom_id="skin_editor_placeholders"))

            save_btn = disnake.ui.Button(label="Save Skin", emoji="<:tec_save:1216777209734369331>", disabled=save_disabled)
            save_btn.callback = self.save
            self.add_item(save_btn)
...
class SelectBotVoice(disnake.ui.View):
    ...
    def build_interactions(self, freebots: List[BotCore] = None):
        ...
        refresh_btn = disnake.ui.Button(label="Update list", emoji="<a:tec_reload:1212800703056060447>", style=disnake.ButtonStyle.blurple)
        refresh_btn.callback = self.reload_callback
        self.add_item(refresh_btn)

        cancel_btn = disnake.ui.Button(label="Cancel", emoji="<a:tec_cross:1213036478259535943>")
