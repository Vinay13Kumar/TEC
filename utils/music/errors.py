...
def parse_error(
    ...
    if isinstance(error, NotDJorStaff):
        ...
        if ctx.command.usage:
            ...
            error_txt += f'<a:tec_book:1216413735330250864> **⠂How to Use:** ```\n{ctx.command.usage.replace("{prefix}", prefix).replace("{cmd}", ctx.command.name).replace("{parent}", ctx.command.full_parent_name)}```\n' \
                        f"<a:tec_warn:1212266812998422528> **⠂Notes about using arguments in the command:** ```\n" \
                        f"[] = Required | <> = Optional```\n"
        ...
        components = [
            disnake.ui.Button(label="Open the favorites manager",
                              custom_id="musicplayer_fav_manager", emoji="<:tec_star:1212069288203255948>"),
