...
class LavalinkPlayer(wavelink.Player):
    ...
    def controller_link(self):
        if self.controller_mode:
            try:
                ...
                try:
                    return f" [`<a:tec_hash:1221376463958315088>`](<{self.text_channel.jump_url}>)"
                except AttributeError:
                    pass
        return ""
            ...
            if not self._new_node_task:
                ...
                try:
                    ...
                    self.set_command_log(text="I noticed an attempt to disconnect me from the channel. "
                                           ...
                                           emoji="<a:tec_warn:1212266812998422528>")
                    self.update = True
    ...
    async def hook(self, event) -> None:
        ...
        if isinstance(event, wavelink.TrackStuck):
            ...
            try:
                self.set_command_log(text=f"The song [{fix_characters(self.current.single_title, 25)}]({self.current.uri}) froze.", emoji="<a:tec_warn:1212266812998422528>")
            except:
                pass
                ...
                if track_data.info["sourceName"] == "spotify" and self.bot.spotify:
                    ...
                    for i in range(3):
                        try:
                            ...
                            self.set_command_log(emoji="<a:tec_warn:1212266812998422528>", text=f"Failed to retrieve recommended songs from Spotify, attempt {i+1} of 3.")
                            self.update = True
                            traceback.print_exc()
                            exception = e
                            await asyncio.sleep(5)
    ...
    async def process_idle_message(self):
        ...
        controller_opts = [
            disnake.SelectOption(
                emoji="<:tec_add:1216398582609874996>", value=PlayerControls.add_song, label="Add song",
                description=f"Play a new song/playlist."
            ),
            disnake.SelectOption(
                emoji="<:tec_star:1212069288203255948>", value=PlayerControls.enqueue_fav, label="Add favorite",
        ...
        if (played := len(self.played)) or self.last_track:
            ...
            controller_opts.extend(
                [
                    disnake.SelectOption(
                        emoji="<:tec_prev:1216400464556462221>", value=PlayerControls.back, label="Back",
                        description=f"Listen again: {play_txt[:31]}"
                    ),
                    disnake.SelectOption(
                        label="Enable autoplay", emoji="<a:tec_reload:1214591681073254410>",
        ...
        if played > 1:
            controller_opts.append(
                disnake.SelectOption(
                    emoji="<:tec_arrow_hook_left:1212066877820371025>", value=PlayerControls.readd, label="Play again",
        ...
        controller_opts.append(
            disnake.SelectOption(
                emoji="<:tec_stop:1216409401217515550>", value=PlayerControls.stop, label="Stop",
        ...
        try:
            if isinstance(self.text_channel.parent, disnake.ForumChannel) and self.static:
                kwargs["content"] = "<a:tec_afk:1213809464545910815> Waiting for new songs..."
        except:
            pass
    ...
    async def idling_mode(self):
        ...
        msg = "<a:tec_afk:1213809464545910815> **⠂The player was disconnected due to inactivity...**"
    ...
    async def update_stage_topic(self, reconnect=True, clear=False):
        ...
        if self.current:
            ...
            if not self.current.is_stream and (not self.auto_pause or not self.paused):
                ...
                timestamp = ("paused" if (self.paused) else "<:tec_pause:1213808106727800862>") + (f" <t:{int(disnake.utils.utcnow().timestamp())}:R>") if not self.current.is_stream else ""
        ...
        if isinstance(self.guild.me.voice.channel, disnake.StageChannel):
            ...
            try:
                ...
                if isinstance(e, disnake.Forbidden):
                    ...
                    self.set_command_log(emoji="<a:tec_cross:1213036478259535943>", text="The automatic status has been deactivated due to a lack of permission to change status.")
                    self.update = True
                print(traceback.format_exc())
    ...
    async def invoke_np(self, force=False, interaction=None, rpc_update=False):
        ...
        try:
            if self.static and isinstance(self.text_channel.parent, disnake.ForumChannel):
                data["content"] = f"`{'<:tec_play:1213064302647320626>' if not self.paused else '<:tec_pause:1213808106727800862>'} {fix_characters(self.current.title, 50)}` |\n\n" + (data.get("content") or "")
        except:
            pass
        ...
        if not self.controller_mode:
            ...
            if data.get("components") is None:
                ...
                data["components"] = [
                    disnake.ui.Button(
                        emoji="<:tec_PlayPause:1216414413922504794>", custom_id=PlayerControls.pause_resume, style=get_button_style(self.paused)),
                    disnake.ui.Button(
                        emoji="<:tec_prev:1216400464556462221>", custom_id=PlayerControls.back),
                    disnake.ui.Button(
                        emoji="<:tec_stop:1216409401217515550>", custom_id=PlayerControls.stop),
                    disnake.ui.Button(
                        emoji="<:tec_next:1216400005972365345>", custom_id=PlayerControls.skip),
                    disnake.ui.Button(
                        emoji="<:tec_queue:1217105407038722108>", custom_id=PlayerControls.queue,
                    ...
                    disnake.ui.Select(
                        ...
                        options=[
                            ...
                            disnake.SelectOption(
                                label="Enqueue favorite", emoji="<:tec_star:1212069288203255948>",
                            ...
                            disnake.SelectOption(
                                label="Seek to start", emoji="<:tec_rewind:1216401126690132190>",
                                value=PlayerControls.seek_to_start,
                                description="Go back to the beginning of the current song."
                            ),
                            disnake.SelectOption(
                                label=f"Volume: {self.volume}%", emoji="<:tec_vol:1215664974622625902>",
                            ...
                            disnake.SelectOption(
                                label=("Disable" if self.nightcore else "Enable") + " nightcore effect", emoji="<a:tec_moon:1220632480340508735>",
                            ...
                            disnake.SelectOption(
                                ...
                                emoji="<a:tec_reload:1214591681073254410>",
                                value=PlayerControls.autoplay,
                            ...
                            disnake.SelectOption(
                                ...
                                emoji="<a:tec_lock:1213021806697648128>",
                    ...
                    try:
                        ...
                        if self.static or self.has_thread:
                            self.set_command_log(
                                ...
                                "<a:tec_warn:1212266812998422528>")
                            self.update = True
                            return
                ...
                try:
                    if self.has_thread:
                        ...
                        await inter.response.edit_message(
                            content=None,
                            embed=disnake.Embed(
                                description=f"<:tec_stop:1216409401217515550> ⠂{self.command_log}",
                                color=self.bot.get_color(self.guild.me)),
                            components=song_request_buttons
                        )
