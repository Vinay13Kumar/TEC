    ...
    async def defer(self, ephemeral: bool = False, *args, **kwargs):
        ...
        if self.bot.config["ENABLE_DEFER_TYPING"]:
            ...
            if self.message.author.id != self.bot.user.id and perms.send_messages and perms.add_reactions:
                await self.message.add_reaction("<a:tec_clock:1217401304347840522>")
        return
...
class EmbedPaginator(disnake.ui.View):
    ...
    @disnake.ui.button(emoji='<:tec_left:1212066251967438929>', style=disnake.ButtonStyle.grey)
    ...
    @disnake.ui.button(emoji='<:tec_right:1220240027959955530>', style=disnake.ButtonStyle.grey)
    ...
    @disnake.ui.button(emoji='<:tec_stop:1216409401217515550>', style=disnake.ButtonStyle.red, label="Close")
...
song_request_buttons = [
    disnake.ui.Button(label="Request a song", emoji="<a:tec_music:1216415601778495558>", custom_id=PlayerControls.add_song),
    disnake.ui.Button(label="Favorite/Integration", emoji="<:tec_star:1212069288203255948>", custom_id=PlayerControls.enqueue_fav)
]
...
async def send_idle_embed(
    ...
    if is_forum:
        content = "<a:tec_music:1216415601778495558> Request your music here."
    else:
        content = None
...
music_source_emoji_data = {
    "youtube": "<a:tec_YouTube:1216417865826046004>",
    "soundcloud": "<a:tec_soundcloud:1216418206235754667>",
    "spotify": "<a:tec_spotify:1216418498692120706>",
    "deezer": "<a:tec_deezer:1216418670624903239>",
    "twitch": "<a:tec_twitch:1216418835129827360>",
}

def music_source_emoji(name: str):
    return music_source_emoji_data.get(name, "<a:tec_play:1213064036606943254>")
...
def music_source_emoji_url(url: str):
    ...
    if url == ">> saved_queue <<":
        return "<:tec_save:1216777209734369331>"

    return "<a:tec_play:1213064036606943254>"
...
async def select_bot_pool(inter: Union[CustomContext, disnake.MessageInteraction, disnake.AppCmdInter], first=False, return_new=False, edit_original=False):
    ...
    if len(bots) == 1 or first:
        ...
        opts = [disnake.SelectOption(label=f"{b.user}", value=f"{b.user.id}", emoji="<a:tec_music:1216415601778495558>") for b in bots.values()]

        opts.append(disnake.SelectOption(label="Cancel", value="cancel", emoji="<a:tec_cross:1213036478259535943>"))
